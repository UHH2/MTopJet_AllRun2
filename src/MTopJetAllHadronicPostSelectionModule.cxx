...
bool MTopJetAllHadronicPostSelectionModule::process(uhh2::Event& event){
  ...
  */
  ...
  if(lowPU){
    if(isMC)h_RecGenHists_allHad_lowPU->fill(event);
    if(isMC)h_RecGenHists_allHad_noJEC_lowPU->fill(event);
  }
  if(midPU){
    if(isMC)h_RecGenHists_allHad_medPU->fill(event);
    if(isMC)h_RecGenHists_allHad_noJEC_medPU->fill(event);
  }
  if(highPU){
    if(isMC)h_RecGenHists_allHad_highPU->fill(event);
    if(isMC)h_RecGenHists_allHad_noJEC_highPU->fill(event);
  }
