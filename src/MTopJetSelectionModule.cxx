...
MTopJetSelectionModule::MTopJetSelectionModule(uhh2::Context& ctx){
  ...
  if     (channel == "muon") channel_ = muon;
    ...
    std::string log("MTopJetSelectionModule::MTopJetSelectionModule -- ");
    log += "MTopJetSelectionModule::MTopJetSelectionModule -- invalid argument for 'channel' key in xml file (must be 'muon' or 'elec'): \""+channel+"\"";
...
bool MTopJetSelectionModule::process(uhh2::Event& event){
  ...
  HFolder("twodcut")->fill(event);//noch "if(lepN == 1)" davor?
  ...
  // jet
  if(!(event.jets->size() >= 2)) throw std::runtime_error("MTopJetSelectionModule::process -- logic error: jet multiplicity < 2 ("+std::to_string(event.jets->size())+")");
  ...
  // ttbar reco hyp
  const ReconstructionHypothesis* rec_ttbar = get_best_hypothesis(ttbar_hyps, "Chi2");
  if(!rec_ttbar) throw std::runtime_error("MTopJetSelectionModule::process -- logic error: ttbar reconstruction hypothesis (\"get_best_hypothesis\", discr=Chi2) not found");
