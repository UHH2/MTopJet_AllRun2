void fit_jetmass(TString mode)
{
  ...
  if(mode == "raw")  hist_title << "XCone_raw_SF/M_jet1";
  if(mode == "jec")  hist_title << "XCone_jec_SF/M_jet1";
  if(mode == "cor")  hist_title << "XCone_cor_SF/M_jet1";
  ...
  bool do_fit = false;
  TF1* fit;
  if(mode == "raw") fit = new TF1("fit", "gaus", 150, 200); 
  if(mode == "jec") fit = new TF1("fit", "gaus", 160, 210); 
  if(mode == "cor") fit = new TF1("fit", "gaus", 150, 200); 
  ...
  all_->SetTitle(" ");
  all_->GetXaxis()->SetTitle("Leading Jet Mass [GeV]");
  ...
  leg = new TLegend(0.55,0.65,0.80,0.75);
  ...
  title << "Fit mean = " << std::setprecision(5) << fit->GetParameter(1)<<" GeV";
  TLatex *t = new TLatex(.55,.6,title.str().c_str()); 
  t->SetTextSize(0.03);
  t->SetNDC(kTRUE);
  t->Draw("");
  std::stringstream title2;
  title2 << "Fit rms    = " << std::setprecision(4) << fit->GetParameter(2)<<" GeV";
  TLatex *t2 = new TLatex(.55,.55,title2.str().c_str()); 
  t2->SetTextSize(0.03);
  t2->SetNDC(kTRUE);
  t2->Draw("");
  std::stringstream title3;
  title3 << "#chi^{2}/ndf = "  << fit->GetChisquare()/fit->GetNDF();
  TLatex *t3 = new TLatex(.55,.5,title3.str().c_str()); 
  t3->SetTextSize(0.03);
  t3->SetNDC(kTRUE);
  t3->Draw("");

  if(mode == "raw") A->SaveAs("/afs/desy.de/user/s/schwarzd/Plots/JetMassFit_raw.pdf");
  if(mode == "jec") A->SaveAs("/afs/desy.de/user/s/schwarzd/Plots/JetMassFit_jec.pdf");
  if(mode == "cor") A->SaveAs("/afs/desy.de/user/s/schwarzd/Plots/JetMassFit_cor.pdf");
