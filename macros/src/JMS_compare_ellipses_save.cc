...
inline void set_graph(TPolyMarker3D *graph, int color, int size);
...
int main(int argc, char* argv[]){
  ...
  JMS_min["combine_cor_syst_2"] = ExtractFromROOT("Graphs/JMS_nominal", "combine", "combine", "syst_2");


  map<TString, TPolyMarker3D*> JMS_1s;
  JMS_1s["combine_cor"]        = ExtractFromROOT("Graphs/JMS_ellipse", "combine", "combine", "");
  JMS_1s["combine_cor_syst_2"] = ExtractFromROOT("Graphs/JMS_ellipse", "combine", "combine", "syst_2");
  ...
  TF2* dummy = new TF2("dummy","x*y", -1.8, 1.8, -1.8, 1.8);
  draw(dummy, {JMS_min["combine_cor"], JMS_min["combine_cor_syst_2"]}, {JMS_1s["combine_cor"], JMS_1s["combine_cor_syst_2"]}, {kRed+2, kBlue+2}, {kOpenTriangleUp, kOpenTriangleDown}, {"No syst", "syst"}, "syst_comparison_cor");
...
inline void set_graph(TPolyMarker3D *graph, int color, int style, int size){
  graph->SetMarkerColor(color);
...
void draw(TF2* dummy, vector<TPolyMarker3D*> min, vector<TPolyMarker3D*> graph, vector<int> color, vector<int> style,  vector<TString> names, TString name){
  ...
  dummy = new TF2("dummy","3*x*y+190", -1.8, 1.8, -1.8, 1.8);
