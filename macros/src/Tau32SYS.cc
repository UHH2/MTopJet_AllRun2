...
int main(int argc, char* argv[]){
  ...
  vector<TString> years = {"2016", "combine"};
  vector<vector<TString>> all  = {
    { "nominal",       "jecup",    "jecdown"},
    { "nominal",       "jmsup",    "jmsdown"},
    { "nominal",     "hdampup",  "hdampdown"},
    { "nominal",      "isrup2",   "isrdown2"},
    { "nominal",      "tuneup",   "tunedown"},
    { "nominal",       "corup",    "cordown"},
    { "nominal",   "gluonmove"              },
    { "nominal",    "qcdbased"              }
  };
  vector<TString> LegendEntries = { "nominal", "JEC up", "JEC down"};
  vector<Color_t> colors =        {    kBlack, kAzure+7,   kAzure+7};
  vector<int>     styles =        {         1,        1,          2};

  int rebin = 5;

  TFile *file = new TFile("files/FSR_hists_pt0toInf.root");

  for(auto year: years){
    double lumi = year.EqualTo("2016")?36.:101.;
    for(unsigned int v=0; v<all.size(); v++){
      ...
      for(TString var: variations){
        TH1F* h = (TH1F*) file->Get(var+"_"+year);
        h->Rebin(5);
      ...
      if(norm) m_can->SaveAs("plots/FSR/SYS/tau32_SYS_"+title+"_"+year+"_norm.pdf");
      else     m_can->SaveAs("plots/FSR/SYS/tau32_SYS_"+title+"_"+year+".pdf");

      TString bracket = (v==0)? "(": (v==all.size()-1)? ")": "";
      if(norm) m_can->SaveAs("plots/FSR/SYS/tau32_SYS_"+year+"_norm.pdf"+bracket);
      else     m_can->SaveAs("plots/FSR/SYS/tau32_SYS_"+year+".pdf"+bracket);
