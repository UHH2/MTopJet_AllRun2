...
int main(int argc, char* argv[]){
  ...
  fstream fit_parameters;
  fit_parameters.open("fits/fit_"+year, ios::out);
  ...
  if(strcmp(year, "2016")==0) is16 = true;
    ...
    cout << "... Sorry, had a long day, but can you please give me the correct year? (2016, 2017, 2018)" << endl;
    cout << "" << endl;
    throw runtime_error("By the way ... it is line 17");
  }

  if(is17 || is18) is1718 = true;
  ...
  for(unsigned int a=0; a<FSRup_general.size();a++){ // not norm and norm (ttbar, data and variations)
    for(unsigned int b=0; b<FSRup_general.at(a).size();b++){ // not rebin and rebin (ttbar, data and variations)
      for(unsigned int c=0; c<FSRup_general.at(a).at(b).size();c++){ // variation factors (sqrt_2, 2, 4)
        ...
        if(a==0){ // not norm
          if(b==0) A->SaveAs(save_path+year+"/tau32_SYS_"+FSR_strings[c]+"_"+year+".pdf");
          if(b==1) A->SaveAs(save_path+year+"/tau32_SYS_"+FSR_strings[c]+"_Rebin10_"+year+".pdf");
        }
        if(a==1){ // norm
          if(b==0) A->SaveAs(save_path+year+"/tau32_SYS_"+FSR_strings[c]+"_norm_"+year+".pdf");
          if(b==1) A->SaveAs(save_path+year+"/tau32_SYS_"+FSR_strings[c]+"_Rebin10_norm_"+year+".pdf");
        }
        delete A;
      }
    }
    ...
    for(unsigned int a=0; a<FSRup_general.size();a++){ // not norm and norm
      for(unsigned int b=0; b<FSRup_general[a].size();b++){ // not rebin and rebin
        ...
        if(a==0){
          ...
          if(b==1) A->SaveAs(save_path+year+"/tau32_SYS_all_Rebin10_"+year+".pdf");
        }
        if(a==1){
          if(b==0) A->SaveAs(save_path+year+"/tau32_SYS_all_norm_"+year+".pdf");
          if(b==1) A->SaveAs(save_path+year+"/tau32_SYS_all_Rebin10_norm_"+year+".pdf");
        }
        leg->Clear();
        delete A;
      }
  ...
  */
  vector<double> FSRup_min = {0, 0}; vector<double> FSRup_min_up = {0, 0}; vector<double> FSRup_min_down = {0, 0};
  for(unsigned int bin=0; bin<8;bin++){ // Starting at Bin 3. For Completeness the first two bins are set to 0.
    double a_par = a[bin];
    double b_par = b[bin];
    FSRup_min.push_back(a_par + b_par * log(minX2));
    FSRup_min_up.push_back(abs(FSRup_min[bin+2]-(a_par + b_par * log(X2_sigmaup)))); // Here and next line: (FSRup_min_up & _down)
    FSRup_min_down.push_back(abs(FSRup_min[bin+2]-(a_par + b_par * log(X2_sigmadown)))); // take minus to get error for TGraphAsymmErrors (below)
  }
  ...
  h_FSRup_min->SetTitle("Minimum Factor of Chi2");
  ...
  FSRdown_general[1].at(1).at(2)->SetLineStyle(2);
  ...
  auto errors = new TGraphAsymmErrors(10, &xbins[0], &FSRup_min[0], &xbins_err[0], &FSRup_min_down[0], &xbins_err[0], &FSRup_min_up[0]);
  ...
  leg->AddEntry(h_FSRup_min, "FSRup","l");
  leg->AddEntry(data_all_norm[1], "data","l");
  leg->SetTextSize(0.02);
  leg->Draw();
  gPad->RedrawAxis();
  C->SaveAs(save_path+year+"/tau32_minimum_factor_"+year+".pdf");
  FSRup_general[1].at(1).at(2)->Draw("SAME HIST");
  FSRdown_general[1].at(1).at(2)->Draw("SAME HIST");
  leg->AddEntry(FSRup_general[1].at(1).at(2), "FSRup_4","l");
  leg->AddEntry(FSRdown_general[1].at(1).at(2), "FSRdown_4","l");
  C->SaveAs(save_path+year+"/tau32_minimum_factor_compare_"+year+".pdf");
  leg->Clear();
  delete C;
}
